#!/usr/bin/env python3
"""
NinjaRMM data collector for es-inventory-hub
"""
import os
import sys
...
from .api import NinjaRMMAPI
from .mapper import NinjaDeviceMapper

logger = get_logger(__name__)


class NinjaCollector:
    """NinjaRMM data collector"""
    
    def __init__(self):
        self.client = NinjaRMMAPI()
        self.mapper = NinjaDeviceMapper()
        self.snapshot_date = datetime.now(timezone.utc)
    
    def collect_sites(self, db: Session) -> Dict[str, Site]:
        """Collect and store sites from NinjaRMM"""
        logger.info("Starting site collection")
        
        try:
            # Get organizations from API (these are our "sites")
            sites_data = self.client.get_organizations()
            logger.info(f"Retrieved {len(sites_data)} organizations from NinjaRMM")
            ...
            for site_data in sites_data:
                ...
                existing_alias = db.query(SiteAlias).filter(
                    SiteAlias.alias_type == 'ninja_site',
                    SiteAlias.external_id == ninja_site_id
                ).first()
                ...
                if existing_alias:
                    ...
                    sites_map[ninja_site_id] = existing_site
                    ...
                    site_alias = SiteAlias(
                        ...
                        external_id=ninja_site_id
                    )
                    db.add(site_alias)
                    sites_map[ninja_site_id] = site
    ...
    def collect_devices(self, db: Session, sites_map: Dict[str, Site]) -> List[Device]:
        """Collect and store devices from NinjaRMM"""
                    ...
                    for device_data in devices_data:
                        try:
                            ...
                            existing_device = db.query(Device).filter(
                                Device.ninja_device_id == device.ninja_device_id
                            ).first()
        ...
        try:
            ...
            for device in devices:
                try:
                    ...
                    device_data = self.client.get_device_details(device.ninja_device_id)
                    ...
                    existing_snapshot = db.query(DeviceSnapshot).filter(
                        ...
                        DeviceSnapshot.source_system == 'ninja'
                    ).first()
    ...
    def run(self) -> None:
        ...
        logger.info("Starting NinjaRMM data collection")
        ...
        try:
            ...
            logger.info("Successfully connected to NinjaRMM API")
