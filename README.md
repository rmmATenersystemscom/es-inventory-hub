...
- **Fresh Data Guarantee**: QuickBooks data updates (invoices, corrections) are now reflected immediately when QBWC runs
- **Session Tracking**: Fixed bug where same period was synced multiple times in one session
- **Bug Fix**: Resolved issue where previously-synced periods wouldn't update even when QB data changed

### Previous Version (v1.38.7)

QBR Metric Descriptions & Dashboard AI Integration:
- **Metric Descriptions**: Added descriptions for all 31 QBR metrics explaining data source and calculations
- **QuickBooks Account Names**: Descriptions include QB account names (e.g., "Managed Services accounts")
- **Dashboard AI Documentation**: Comprehensive API reference at `/prompts/qbr-api-reference.md`
- **Year-End Totals**: Documented which metrics should show totals vs point-in-time values

### Previous Version (v1.38.6)
